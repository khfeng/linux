...
)
{
	...
	if (hw_chplan != EEPROM_DEFAULT_CHANNEL_PLAN)
		rtw_sku_set_det_flag(padapter, RTW_DETF_CH_PLAN);

chk_hw_country_code:
	if (hw_alpha2 && !IS_ALPHA2_NO_SPECIFIED(hw_alpha2)) {
		rtw_sku_set_det_flag(padapter, RTW_DETF_EFUSE_COUNTRY);
...
chk_sw_config:
	...
	if (rtw_sku_start_det_if_need(padapter)) {
		country_ent = NULL;
		/* 2G: 1~13, 5G: none, pwr limit: WW */
		chplan = RTW_CHPLAN_WORLD_NULL;
	}
