...
s8 PHY_GetTxPowerLimitSAR(_adapter *adapter
	, const char *regd_name
	, BAND_TYPE band, CHANNEL_WIDTH bw
	, u8 rfpath, u8 rate, u8 ntx_idx, u8 cch
)
{
	...
	limit_sar = smbios_get_tx_power_limit_sar(adapter, rfpath, band, cch);

	if (limit_sar == MAX_POWER_INDEX)
	...
	/* return diff value */
	limit_sar = limit_sar - PHY_GetTxPowerByRateBase(adapter, band, rfpath, rs);

	return limit_sar;
}
#endif /* CONFIG_TXPWR_LIMIT_SMBIOS_SAR */
...
void dump_tx_power_idx_title(void *sel, _adapter *adapter)
{
	...
	RTW_PRINT_SEL(sel, "%-4s %-9s %2s %-3s %-4s %-3s %-3s %-3s %-4s %-3s %-5s\n"
		, "path", "rate", "", "pwr", "base", "", "(byr", "lmt", "sar)", "tpt", "ebias");
}
...
void dump_tx_power_idx_by_path_rs(void *sel, _adapter *adapter, u8 rfpath, u8 rs)
{
	...
	for (i = 0; i < rates_by_sections[rs].rate_num; i++) {
		...
		RTW_PRINT_SEL(sel, "%4c %9s %uT %3u %4u %3d (%3d %3d %3d) %3d %5d\n"
			, rf_path_char(rfpath), MGN_RATE_STR(rates_by_sections[rs].rates[i]), tic.ntx_idx + 1
			, power_idx, tic.base, rtw_min(rtw_min(tic.by_rate, tic.limit), tic.limit_sar), tic.by_rate, tic.limit, tic.limit_sar, tic.tpt, tic.ebias);
	}
}
...
post_hdl:
	...
	rtw_txpwr_init_regd(rfctl, rfctl->country_ent, rfctl->ChannelPlan, 0);
