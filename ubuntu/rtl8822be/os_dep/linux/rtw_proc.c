...
static int proc_get_smbios_info(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);

	return smbios_info(adapter, m);
}
#endif

#ifdef CONFIG_RTW_SKU
static int proc_get_sku_info(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);

	return rtw_sku_dump_info(adapter, m);
}

ssize_t proc_set_sku_info(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	char tmp[32];
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%u", &expire_age);

		if (num != 1)
			return count;

		if (expire_age)
			rtw_sku_set_expired_age(padapter);
	}
...
#ifdef CONFIG_TXPWR_LIMIT_SMBIOS_SAR
	RTW_PROC_HDL_SSEQ("smbios", proc_get_smbios_info, NULL),
#endif
#ifdef CONFIG_RTW_SKU
	RTW_PROC_HDL_SSEQ("sku", proc_get_sku_info, proc_set_sku_info),
#endif
};
