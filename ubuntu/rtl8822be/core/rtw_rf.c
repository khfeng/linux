...
static struct country_chplan country_chplan_sku;

const struct country_chplan *
insersect_country_ent(const struct country_chplan *old_ent,
		      const struct country_chplan *new_ent,
		      u8 intersect)
{
	if (!intersect)
		return new_ent;

	if (old_ent != &country_chplan_sku) {
		country_chplan_sku = *old_ent;

		/* "UD": user define */
		country_chplan_sku.alpha2[0] = 'U';
		country_chplan_sku.alpha2[1] = 'D';
	}

#ifdef CONFIG_80211AC_VHT
	country_chplan_sku.en_11ac &= new_ent->en_11ac;
#endif

#if RTW_DEF_MODULE_REGULATORY_CERT
	country_chplan_sku.def_module_flags &= new_ent->def_module_flags;
#endif

	return &country_chplan_sku;
}
...
int regd_id(const char *str)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(_regd_str); i++) {
		if (strcmp(_regd_str[i], str) == 0)
			return i;
	}
...
void dump_txpwr_lmt(void *sel, _adapter *adapter)
{
	...
	if (rfctl->regd_ww_mask) {
		struct txpwr_lmt_ent *ent;
		_list *cur, *head;

		RTW_PRINT_SEL(sel, "WW regd extension info:\n");
		...
		while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
			...
			_RTW_PRINT_SEL(sel, "%s%s\tregd_id=%d\n"
				, strcmp(ent->regd_name, rfctl->regd_name) == 0 ? "*" : ""
				, ent->regd_name, ent->regd_id
			);
		}

		_RTW_PRINT_SEL(sel, "%s%s\tmask=%x\n"
				, strcmp(rfctl->regd_name, regd_str(TXPWR_LMT_WW)) == 0 ? "*" : ""
				, regd_str(TXPWR_LMT_WW), rfctl->regd_ww_mask
			);
...
#ifdef CONFIG_RTW_SKU
void rtw_sku_set_det_flag(_adapter *adapter, enum rtw_sku_detf flag)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);

	rfctl->sku.detflags |= flag;
}

static void rtw_sku_set_state(struct rf_ctl_t *rfctl, enum rtw_sku_state state)
{
	rfctl->sku.state = state;

	if (state == RTW_SKU_ST_DETED)
		rfctl->sku.age = rtw_get_current_time();
}

bool rtw_sku_check_if_deting(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);

	switch (rfctl->sku.state) {
	case RTW_SKU_ST_UNDET:
	case RTW_SKU_ST_DETING:
		return true;
...
bool rtw_sku_start_det_if_need(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	bool ret;

	ret = rfctl->sku.detflags == 0;

	if (ret)
		rtw_sku_set_state(rfctl, RTW_SKU_ST_UNDET);
	else
		rtw_sku_set_state(rfctl, RTW_SKU_ST_NONE);

	return ret;
}

static bool _rtw_sku_check_if_exp_scan(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);

	if (rfctl->sku.detflags != 0)
		return false;

	switch (rfctl->sku.state) {
	case RTW_SKU_ST_NONE:
	case RTW_SKU_ST_DETING:
		return false;
	...
	case RTW_SKU_ST_DETED:
		if (rtw_get_passing_time_sec(rfctl->sku.age) <
		    RTW_SKU_LEASE_AGE)
			return false;
		break;
	}
...
bool rtw_sku_check_if_exp_scan(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	bool ret;

	ret = _rtw_sku_check_if_exp_scan(adapter);

	if (ret)
		rtw_sku_set_state(rfctl, RTW_SKU_ST_DETING);

	return ret;
}

void rtw_sku_confirm_detection(_adapter *adapter)
{
	_irqL irqL;
	_list *plist, *phead;
	...
	if (rfctl->sku.state != RTW_SKU_ST_DETING)
		return;
	...
	while (1) {
		...
		if (!pnetwork->network.country_11d)
			goto next;

		/* check if exist? */
		map_bit = pnetwork->network.country_11d - country_chplan_map;
		map_idx = map_bit >> 3;
		map_bit &= 0x7;

		if (map_idx <= sizeof(bitmap)) {
			if (bitmap[map_idx] & (1 << map_bit))
				goto next;

			bitmap[map_idx] |= (1 << map_bit);
		}

		/* send channel plan */
		rtw_set_country_entry_cmd(adapter, RTW_CMDF_DIRECTLY,
					  pnetwork->network.country_11d,
					  first ? _FALSE : _TRUE);
...
next:
	...
	rtw_sku_set_state(rfctl, RTW_SKU_ST_DETED);
}

bool rtw_sku_check_and_apply_regd(_adapter *adapter, const char *alpha2)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	static const struct {
		char alpha2[3];
	...
	bool found = false;
	const struct country_chplan *country;
	int i;

	for (i = 0; i < ARRAY_SIZE(regd_list); i ++ ) {
		if (regd_list[i].alpha2[0] == alpha2[0] &&
		    regd_list[i].alpha2[1] == alpha2[1]) {
			found = true;
			break;
		}
	}
	...
	if (found) {
		rtw_sku_set_det_flag(adapter, RTW_DETF_SPECIAL_COUNTRY);
		rtw_sku_set_state(rfctl, RTW_SKU_ST_NONE);

		country = rtw_get_chplan_from_country(alpha2);
		if (country)
			rtw_set_country_entry_cmd(adapter, RTW_CMDF_DIRECTLY,
						  country, _FALSE);
	}
...
int rtw_sku_dump_info(_adapter *adapter, void *sel)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);

	_RTW_PRINT_SEL(sel, "SKU:\nflags: %#08x\nState: %d\n",
		       rfctl->sku.detflags, rfctl->sku.state);

	_RTW_PRINT_SEL(sel, "age: %#08x (now: %#08x, pass: %d sec, expire: %d sec)\n",
		       rfctl->sku.age,
		       rtw_get_current_time(),
		       rtw_get_passing_time_sec(rfctl->sku.age),
		       RTW_SKU_LEASE_AGE -
				rtw_get_passing_time_sec(rfctl->sku.age));
	return 0;
}

void rtw_sku_set_expired_age(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);

	rfctl->sku.age = rtw_get_current_time() -
			 rtw_sec_to_systime(RTW_SKU_LEASE_AGE);
}
#endif /* CONFIG_RTW_SKU */
