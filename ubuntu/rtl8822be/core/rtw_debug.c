...
int proc_get_survey_info(struct seq_file *m, void *v)
{
	...
	u32 wpsielen = 0;
	char country[3];
	char scan_mode_str[3] = {'P', 'A', 'N'}; /* enum _RT_SCAN_TYPE */
	char scan_mode;
	...
	RTW_PRINT_SEL(m, "%5s  %-17s  %3s  %-3s  %-4s  %-4s  %5s  %32s  %32s  %2s  %1s\n", "index", "bssid", "ch", "RSSI", "SdBm", "Noise", "age", "flag", "ssid", "CC", "M");
	while (1) {
		...
		if (pnetwork->network.country_11d) {
			_rtw_memcpy(country, pnetwork->network.country_11d->alpha2, 2);
			country[2] = '\0';
		} else {
			strcpy(country, "");
		}

		scan_mode = (pnetwork->network.scan_mode <
			     ARRAY_SIZE(scan_mode_str) ?
			     scan_mode_str[pnetwork->network.scan_mode] : '?');
		...
		sprintf(flag_str, "%s%s%s%s%s%s%s",
			...
			(ie_cap & BIT(0)) ? "[ESS]" : "",
			(ie_p2p) ? "[P2P]" : "");
		RTW_PRINT_SEL(m, "%5d  "MAC_FMT"  %3d  %3d  %4d  %4d    %5d  %32s  %32s  %2s  %c\n",
			      ++index,
