...
static u8 intersect_channel_set(u8 ch_nr_in1,
				const RT_CHANNEL_INFO *channel_set_in1,
				u8 ch_nr_in2,
				const RT_CHANNEL_INFO *channel_set_in2,
				RT_CHANNEL_INFO *channel_set_out)
{
	u8 ch_nr_out = 0;
	bool next_in1;
	bool next_in2;

	while (ch_nr_in1 > 0 && ch_nr_in2 > 0) {
		next_in1 = true;
		next_in2 = true;

		if (channel_set_in1->ChannelNum < channel_set_in2->ChannelNum) {
			next_in2 = false;
			goto next;
		}
		if (channel_set_in1->ChannelNum > channel_set_in2->ChannelNum) {
		...
		/* channel is the same, so copy it */
		*channel_set_out = *channel_set_in1;

		/* The precedence: SCAN_NOREPORT -> SCAN_PASSIVE -> ... */
		if (channel_set_in1->ScanType == SCAN_NOREPORT ||
		    channel_set_in2->ScanType == SCAN_NOREPORT)
			channel_set_out->ScanType = SCAN_NOREPORT;
		else if (channel_set_in1->ScanType == SCAN_PASSIVE ||
			 channel_set_in2->ScanType == SCAN_PASSIVE)
			channel_set_out->ScanType = SCAN_PASSIVE;
...
next:
	...
	if (ch_nr_out < MAX_CHANNEL_NUM)
		channel_set_out->ChannelNum = 0; /* add a terminator */
	...
	if (b2_4GBand) {
		...
		for (index = 0; index < CH_LIST_LEN(RTW_ChannelPlan2G[Index2G]); index++) {
			...
			if (ChannelPlan == RTW_CHPLAN_GLOBAL_DOAMIN
				...
				|| (Index2G == RTW_RD_2G_WORLD && !rtw_sku_check_if_deting(padapter))
			) {
...
static u8 init_channel_set_with_intersect(_adapter *padapter, u8 ChannelPlan,
					  u8 ch_nr_in,
					  RT_CHANNEL_INFO *channel_set_inout,
					  u8 intersect)
{
	RT_CHANNEL_INFO channel_set_inbak[MAX_CHANNEL_NUM];
	u8 ch_nr_out;

	if (intersect)
		memcpy(channel_set_inbak, channel_set_inout,
		       sizeof(*channel_set_inout) * ch_nr_in);

	ch_nr_out = init_channel_set(padapter, ChannelPlan, channel_set_inout);

	if (intersect)
		ch_nr_out = intersect_channel_set(ch_nr_in, channel_set_inbak,
						  ch_nr_out, channel_set_inout,
						  channel_set_inout);
			 ...
			 u8 ChannelPlan, u8 intersect)
{
	...
	exc = _rtw_regd_exc_search(rfctl
		, country_ent ? country_ent->alpha2 : NULL
		, ChannelPlan);
	if (exc) {
	...
	if (ChannelPlan == RTW_CHPLAN_REALTEK_DEFINE)
		...
		regd = RTW_ChannelPlanMap[ChannelPlan].regd;
...
release_lock:
	...
	if (ent) {
		if (intersect) {
			rfctl->regd_name = regd_str(TXPWR_LMT_WW);
			rfctl->regd_ww_mask |= BIT(ent->regd_id);
		} else {
			rfctl->regd_ww_mask = BIT(ent->regd_id);
		}
	}
...
static void process_80211d(PADAPTER padapter, WLAN_BSSID_EX *bssid)
{
	...
	if (pregistrypriv->enable80211d
		...
		p += 3;
		RTW_DBG("%s: 802.11d country=%s\n", __FUNCTION__, country);

		if (pregistrypriv->enable80211d == 2) {
			bssid->country_11d = rtw_get_chplan_from_country(country);
...
void report_survey_event(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	psurvey_evt->bss.scan_mode = ss->scan_mode_final;
...
static int rtw_sku_scan_expand(_adapter *adapter,
			       struct rtw_ieee80211_channel *ch, u8 ch_num)
{
	int i;

	if (!rtw_sku_check_if_exp_scan(adapter))
		return ch_num;

	/* 2G: ch 14 */
	ch[ch_num].hw_value = 14;
	ch[ch_num].scan_mode_override = SCAN_NOREPORT;
	ch_num++;

	for (i = 0; i < CENTER_CH_5G_20M_NUM; i++) {
		ch[ch_num].hw_value = center_ch_5g_20m[i];
		ch[ch_num].scan_mode_override = SCAN_NOREPORT;
		...
		u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num)
{
	...
	j = rtw_sku_scan_expand(padapter, out, j);
...
static u8 sitesurvey_pick_ch_behavior(_adapter *padapter, u8 *ch, RT_SCAN_TYPE *type)
{
...
#endif /* CONFIG_P2P */
	{
		...
		if (ss->channel_idx < ss->ch_num) {
			...
			scan_ch = ch->hw_value;
			if (ch->scan_mode_override)
				scan_type = ch->scan_mode_override;
			else
...
void site_survey(_adapter *padapter, u8 survey_channel, RT_SCAN_TYPE ScanType)
{
	...
	pmlmeext->sitesurvey_res.scan_mode_final = ScanType;
...
*/
	...
	case SCAN_COMPLETE:
		...
		rtw_sku_confirm_detection(padapter);
	}
...
u8 set_chplan_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	country_ent = setChannelPlan_param->country_ent;
	channel_plan = setChannelPlan_param->channel_plan;
	intersect = setChannelPlan_param->intersect;

	country_ent = insersect_country_ent(rfctl->country_ent, country_ent,
					    intersect);

	rfctl->country_ent = country_ent;
	rfctl->ChannelPlan = channel_plan;

	rfctl->max_chan_nums = init_channel_set_with_intersect(
						padapter, rfctl->ChannelPlan,
						rfctl->max_chan_nums,
						rfctl->channel_set, intersect);
	init_channel_list(padapter, rfctl->channel_set, &rfctl->channel_list);
#ifdef CONFIG_TXPWR_LIMIT
	rtw_txpwr_init_regd(rfctl, country_ent, channel_plan, intersect);
#endif
