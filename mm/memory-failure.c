...
static void __add_to_kill(struct task_struct *tsk, struct page *p,
			  ...
			  unsigned long addr, unsigned long size_shift)
{
	struct to_kill *tk;
		...
		unsigned long addr)
{
	if (addr == -EFAULT)
		return;
	__add_to_kill(tsk, p, vma, to_kill, addr, 0);
}
			      ...
			      struct list_head *to_kill, pgoff_t pgoff)
{
	...
	unsigned long size_shift = !(pfn_valid(pgoff)) && (vma->vm_flags | PFN_MAP) ?
				  PAGE_SHIFT : 0;
	__add_to_kill(tsk, p, vma, to_kill, addr, size_shift);
}
